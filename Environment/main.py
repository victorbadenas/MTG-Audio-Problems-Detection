...
def multi_file_compute(audiofolder: str, jsonfolder: str):
    ...
    if os.path.exists(tsvpath):
        os.remove(tsvpath)  # cleaning from previous executions

    with tqdm(os.listdir(audiofolder)) as pb:
        for file in os.listdir(audiofolder):
            ...
            if extension == ".wav":
                ...
                json_dict = single_json_compute(audiopath, jsonfolder, printFlag=False)
...
def create_files_arrays(tsvpath: str):
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Calls the audio_problems_detection algorithms and stores the result in a json file")
    parser.add_argument("audiofolder", help="relative path to the audiofile")
    parser.add_argument("--jsonfolder", help="string containing the relative path for the json file", default="",
                        required=False)
