...
def compute_histogram(idx_arr: list, xticks: list, mask=[]):
    ...
    if len(mask) == 0:
        ...
        hist = np.zeros(len(xticks))
        if len(mask) != len(idx_arr):
            raise ValueError("Inconsistent mask size")
        for i, boolean in zip(idx_arr, mask):
            if boolean:
                hist[i] += 1
        return hist
...
def get_last_true_index(bool_list: list):
    ...
    if not any(bool_list):
        return len(bool_list)-1

    for i, item in enumerate(reversed(bool_list)):
        if item:
            return len(bool_list)-1-i
...
def detectBW(fpath: str, frame_size: float, hop_size: float, floor_db: float, oversample_f: int):
    ...
    avg_frames /= (i+1)
    most_likely_bin, conf, binary = compute_mean_fc(avg_frames, fc_index_arr, [], SR, hist=hist)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--frame_size", help="frame_size for the analysis fft (default=256)",
                        default=1024, required=False)
    parser.add_argument("--hop_size", help="hop_size for the analysis fft (default=128)", default=512, required=False)
    ...
    parser.add_argument("--oversample",
                        help="(int) factor for the oversampling in frequency domain. Must be a power of 2",
